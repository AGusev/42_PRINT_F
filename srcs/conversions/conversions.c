...
/*   Updated: 2019/03/29 14:37:35 by agusev           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char		*do_flags(char *return_conv, t_flags *flags, int *length, char **str)
{
	...
	{
		return_conv = ft_update(return_conv, adding_precision_string(flags, return_conv));
		return_conv = ft_update(return_conv, adding_precision_number(flags, return_conv));
	}
	if (flags->minus == 1)
...
char		*continue_conversion(t_flags *flags, va_list *arg, char *return_conv)
{
	...
	if (flags->conversion == 'f')
	{
		...
		if (flags->zero_padding == 1 && return_conv != 0)
		{
			...
			{
				return_conv = ft_update(return_conv, ft_strjoin(filler('0', flags->width - ft_strlen(return_conv)), return_conv));
			}
		}
	}
	return (return_conv);
...
char		*start_conversion(t_flags *flags, va_list *arg, int *length, char **str)
{
	...
	if (flags->conversion == 'C')
		return_conv = ft_update(return_conv, conversion_wide_c(arg, length, flags));
	...
	if (flags->conversion == 'D' || flags->conversion == 'u' || \
		flags->conversion == 'U')
		return_conv = ft_update(return_conv, conversion_u_cap_d_cap_u(arg, flags));
